                    ...
                    <td>
                        @if (isset($column['type']))
                            @if ($column['type'] == 'image')
                                <img src="{{ $value }}" style="height: 2.5rem;">
                            @endif
                            ...
                            @if ($column['type'] == 'json_images')
                                @php
                                    $i = 1;
                                @endphp
                                @foreach (json_decode($value??"[]") as $image)
                                    <a href="{{asset('storage/'.$image)}}" target="_blank" style="max-height: 50px; max-width: 50px;">عرض المرفق{{$i}}</a>
                                    @if ($i < count(json_decode($value??"[]"))) , @endif
                                    @php $i++; @endphp
                                @endforeach
                            @endif
                        @else
